...
# Segments follow the supplied video's visible labels. Headshot (0-1.29) is intentionally unused.
$edges=@(1.29,2.656,4.123,5.556,6.956,8.723,10.623,12.886)
for($i=0;$i -lt 7;$i++) {
    ...
    & ffmpeg -hide_banner -loglevel error -ss $start -i $VideoPath -t $length -vn -ac 1 -ar 22050 -c:a pcm_s16le -af "afade=t=in:d=0.004,afade=t=out:st=${fade}:d=0.008" -y (Join-Path $audioRoot ('streak-'+($i+2)+'.wav'))
